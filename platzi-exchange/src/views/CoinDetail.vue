...
export default {
  ...
  methods: {
    async getWebsite(exchange) {
      ...
      const res = await api.getExhange(exchange.exchangeId);
      const url = await res.exchangeUrl;
      this.$set(exchange, "url", url);
      await this.$set(exchange, "isLoading", false);
    ...
    async getCoin() {
      ...
      const promesa = await Promise.all([
        api.getAsset(id),
        api.getAssetHisroty(id),
        api.getMarkets(id)
      ]);
      const asset = await promesa[0];
      const history = await promesa[1];
      const markets = await promesa[2];
      this.asset = asset;
